...
const MobileButtons: FC<IMobileButtonsProps> = (props: IMobileButtonsProps) => {
	return <div className="mobile-buttons">
		...
		<div className="button-row">
			<button className="left-button" type="button" onClick={ () => props.handleMobileButton(PlayerResultEnum.SPACE_BAR) }>SWAP</button>
			<button type="button" onClick={ () => props.handleMobileButton(PlayerResultEnum.ENTER) }>DROP</button>
		</div>
	</div>
}
